    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        ...
        if( ( photon_gamma[ipart] > 2. ) && ( photon_chi[ipart] > chiph_threshold_ ) ) {
            ...
            // If epsilon_tau_ > 0
                ...
                //[SFQEDtoolkit]
                temp = SFQED_BREIT_WHEELER_rate(photon_gamma [ipart], photon_chi[ipart]);
...
// withou tasks
                    ...
                    //[SFQEDtoolkit]
                    //the following relations hold because the emission is assumed collinear
                    double nrgs[2], ps[2];
                    nrgs[0] = SFQED_BREIT_WHEELER_ELECTRON_energy(photon_gamma[ipart], photon_chi[ipart], random_number);
                    
                    ps[0] = std::sqrt( std::pow( nrgs[0], 2 ) - 1 );
                    ps[1] = photon_gamma[ipart] - ps[0];
                    
                    nrgs[1] = std::sqrt( std::pow( ps[1], 2 ) + 1 );
                    ...
                    for( int ipair=i_pair_start; ipair < i_pair_start+mBW_pair_creation_sampling_[0]; ipair++ ) {
                        ...
                        //[classic Smilei]
                        // Momentum
                        // const double p = std::sqrt( std::pow( nrgs[0], 2 ) - 1 );
                        pair0_momentum_x[ipair] = ps[0]*ux;
                        pair0_momentum_y[ipair] = ps[0]*uy;
                        pair0_momentum_z[ipair] = ps[0]*uz;

                        //[SFQEDtoolkit]
                        //compute the particle quantum parameter
                        pair_chi[0] = compute_chi_with_components(nrgs[0],
                                        pair0_momentum_x[ipair], pair0_momentum_y[ipair], pair0_momentum_z[ipair],
                                        Ex[ipart-ipart_ref], Ey[ipart-ipart_ref], Ez[ipart-ipart_ref],
                                        Bx[ipart-ipart_ref], By[ipart-ipart_ref], Bz[ipart-ipart_ref] );
            ...
            //               + new_pair[k].momentum(i,ipair)*remaining_dt*inv_gamma;
                    ...
                    for( auto ipair=i_pair_start; ipair < i_pair_start + mBW_pair_creation_sampling_[1]; ipair++ ) {
                        ...
                        pair1_momentum_x[ipair] = ps[1]*ux;
                        pair1_momentum_y[ipair] = ps[1]*uy;
                        pair1_momentum_z[ipair] = ps[1]*uz;

                        //[SFQEDtoolkit]
                        //compute the particle quantum parameter
                        pair_chi[1] = compute_chi_with_components(nrgs[1],
                                        pair1_momentum_x[ipair], pair1_momentum_y[ipair], pair1_momentum_z[ipair],
                                        Ex[ipart-ipart_ref], Ey[ipart-ipart_ref], Ez[ipart-ipart_ref],
                                        Bx[ipart-ipart_ref], By[ipart-ipart_ref], Bz[ipart-ipart_ref] );
            ...
            //               + new_pair[k].momentum(i,ipair)*remaining_dt*inv_gamma;
                    ...
                    for( int k=0 ; k < 2 ; k++ ) {
                        ...
                        for( int idNew=nparticles-mBW_pair_creation_sampling_[k]; idNew<nparticles; idNew++ ) {
                            ...
                            new_pair_per_bin[ibin][k].momentum( 0, idNew ) = ps[k]*ux ;
                            new_pair_per_bin[ibin][k].momentum( 1, idNew ) = ps[k]*uy ;
                            new_pair_per_bin[ibin][k].momentum( 2, idNew ) = ps[k]*uz ;

                            //[classic Smilei]
                            pair_chi[k] = compute_chi_with_components(nrgs[k],
                                        new_pair_per_bin[ibin][k].momentum( 0, idNew ),
                                        new_pair_per_bin[ibin][k].momentum( 1, idNew ),
                                        new_pair_per_bin[ibin][k].momentum( 2, idNew ),
                                        Ex[ipart-ipart_ref], Ey[ipart-ipart_ref], Ez[ipart-ipart_ref],
                                        Bx[ipart-ipart_ref], By[ipart-ipart_ref], Bz[ipart-ipart_ref] );
