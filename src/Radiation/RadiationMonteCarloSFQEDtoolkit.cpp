...
// ---------------------------------------------------------------------------------------------------------------------
RadiationMonteCarloSFQEDtoolkit::RadiationMonteCarloSFQEDtoolkit( Params &params, Species *species, Random * rand  )
...
// ---------------------------------------------------------------------------------------------------------------------
RadiationMonteCarloSFQEDtoolkit::~RadiationMonteCarloSFQEDtoolkit()
{
}
...
// ---------------------------------------------------------------------------------------------------------------------
void RadiationMonteCarloSFQEDtoolkit::operator()(
    Particles       &particles,
    Particles       *photons,
    SmileiMPI       *smpi,
    ...
    int             ipart_ref)
{
    ...
    for( int ipart=istart ; ipart<iend; ipart++ ) {
        ...
        while( ( local_it_time < dt_ )
            ...
            // [SFQEDtoolkit modification]
            if( true //( particle_chi > RadiationTables.getMinimumChiDiscontinuous() )
            ...
            if( tau[ipart] > epsilon_tau_ ) {
                ...
                //[from SFQEDtoolkit]
                //***********************************************
                temp = SFQED_INV_COMPTON_rate(particle_gamma, particle_chi);
                //***********************************************
                ...
                if( tau[ipart] <= epsilon_tau_ ) {
                    ...
                    // [from SFQEDtoolkit]
                    //***********************************************
                    double photon_gamma = SFQED_LCFA_INV_COMPTON_PHOTON_energy(particle_gamma, particle_chi, random_number);
                    ...
                    //define new photon momentum's components
                    double phtn_mom_x, phtn_mom_y, phtn_mom_z;

                    //rescale the momentum components 
                    // to fit the proper photon energy
                    phtn_mom_x = inv_old_norm_p * momentum_x[ipart];
                    phtn_mom_y = inv_old_norm_p * momentum_y[ipart];
                    phtn_mom_z = inv_old_norm_p * momentum_z[ipart];
